...
#keyboard-joystick constants
shutdownbutton_holdlimit = 200 #how long to hold button before  shutdown 
...
def GPIOSetup():
    ...
    GPIO.setup(16, GPIO.IN)    # Button FIRE
    GPIO.setup(12, GPIO.IN)    # Button RELOAD
    GPIO.setup(7, GPIO.IN)     # Button SHUTDOWN

#function to  read joystick
# returns the direction where it should move
def JoystickRead():
    ...
    if R == 1 and L == 0 and U == 0 and D == 0:
        ...
        return "Move_DOWN"
    elif R == 1 and L == 0 and U == 1 and D == 0:
        return "Move_UR"
    elif R == 0 and L == 1 and U == 1 and D == 0:
        return "Move_UL"
    elif R == 1 and L == 0 and U == 0 and D == 1:
        return "Move_DR"
    elif R == 0 and L == 1 and U == 0 and D == 1:
        return "Move_DL"
    elif Fire: 
        return "Kickback"
    elif Reload: 
...
def main():
    ...
    while going:
        ...
        ###joystick and buttons processing
        inputread = JoystickRead()
        
        if inputread == "reload":
            soldier.order = inputread
        elif inputread == "Kickback":
            sight.status = inputread
            if sight.numberOfbullets >0:Global_SightTriggerPulled = True            
            else:Global_SightTriggerPulled = False
            
        elif inputread == "Shutdown":
            
            if shutdownbutton_holdcounter != shutdownbutton_holdlimit and shutdownbutton_holdcounter != -1:
                shutdownbutton_holdcounter+=1
            elif shutdownbutton_holdcounter == shutdownbutton_holdlimit and shutdownbutton_holdcounter != -1:
                Shutdown()
                shutdownbutton_holdcounter = -1
